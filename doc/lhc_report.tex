...
The dataset is provided by CERN and contains roughly 800,000 proton-proton collision events from the official ATLAS detector simulator. It includes 16 primitive variables measured by the detector, 13 variables derived from the primitive variables, the event classifications, and 4 columns of auxiliary data such as sample weights. 
...
\subsection{SVM}
SVM algorithms offer the potential to use the ``kernel-trick'', where the data is projected into a higher dimensional space before finding optimal boundary separating 
