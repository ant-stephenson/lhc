...
In the following markdown notebooks we use methods from SM1 and SC1 to attempt the Kaggle Challenge in R. We chose to follow a similar structure, using the Kaggle training set to train our model, and the private leaderboard set as our hold-out validation set. 
...
The three groups have varying ratios of signal and background events (approx 1:3, 1:2, and 1:1), though they are similar in size (approx 80,000) and in ratio of weights (approx 1:500).
