...
# SVM 
In the `lhc` package we have implemented two support vector machine (SVM) algorithms: `svm` and `kernel_svm`. In this section of the notebook we attempt to apply these methods to the Higgs Challenge. We would expect soft-margin SVM to have a similar performance to logistic regression, and that kernel SVM could improve on this as it can transform the dataset and create a non-linear decision boundary. Unfortunately, SVM algorithms do not scale well with the number of samples and so we are unlikely to be able to train our models on a significant portion of the training data.

Our SVM functions take as inputs: a design matrix $X$, binary class labels $y \in \{-1, 1\}$, a parameter $C$ to controls the relative weighting of maximising the margin vs minimising the slack variables, and for `kernel_svm` a kernel function `ckernel(x_i, x_j)`. 

The data is restructured as a Lagrangian
...
For `kernel_svm`, $\textbf{XX}^T$ is replaced with $K$, where $K_{ij} = k(\textbf{x}_i, \textbf{x}_j)$.

To solve the quadratic optimisation problem, `solve.QP` from the `quadprog` package is used. The SVM functions then return a function/trained model which predicts the classes of new samples. For example, the prediction function for the soft-margin SVM takes the form $y_i = \textbf{w}^T \textbf{x}_i + w_0$.

SVM algorithms are not probabilistic; that is, they return the predicted class of samples $\hat{y}_i$ not the probability $P(y_i=s | \textbf{x}_i)$. This means we cannot us our `ROC_curve` and `AMS_data` object classes which vary a decision threshold, and we so calculate a single AMS value for the test set.
...
for(g in 1:G){
  ...
  for(k in 1:length(kernels)){
    ...
    #scale data otherwise poly kernel has too large values
    X_test_scaled <- scale_dat(X_test, X_train, add.intercept = F)
    X_train_scaled <- scale_dat(X_train, X_train, add.intercept = F)
...
The linear kernel outperforms the kernel SVM for all groups. This is likely because we have not tuned the hyperparameters of the kernels and the training set is too small for the complexity of the model.
