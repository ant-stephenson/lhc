...
for (mj in 1:G) {
    j <- jet_cats[mj]
    features_to_rm[[mj]] <- colnames(X)[colSums(is.na(X[idx_jet_cat(nj, j) & idx_higgs_mass(X, mj, G),])) > 0]
}
```
...
for (mj in 1:G) {
    ...
    test_row_idx <- idx_jet_cat(nj, j) & idx_higgs_mass(X, mj, G)
...
write.table(as.data.frame(result), file = sprintf("results%i.csv", 1), append = TRUE, sep = ",",
