...
for(j in 1:6){
  ...
  #get a plot of the k ROC curves on the same axis
  full_rocs[[get_model_idx(j, 1, 1)]]$plot_curve(title=sprintf("All training data, %i-fold CV, Average AUC %.3f", K, round(full_rocs[[j]]$auc, 3)))
}
aucb <- sapply(full_rocs, function(x) x$auc)
```
